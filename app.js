...
function renderWasteBins(d) {
  ...
  const BINS = [
    { key: 'restmuell',   lblId: 'bin-restmuell-date' },
    { key: 'gruengut',    lblId: 'bin-gruengut-date' },
    { key: 'gelber_sack', lblId: 'bin-gelber-sack-date' },
  ];

  let anyVisible = false;

  for (const bin of BINS) {
    const lbl   = document.getElementById(bin.lblId);
    if (!lbl) continue;
    const binEl = lbl.closest('.waste-bin');
    const next  = d[bin.key];
    lbl.classList.remove('bin-today', 'bin-tomorrow');

    if (next === today) {
      lbl.textContent = 'heute';
      lbl.classList.add('bin-today');
      binEl.style.display = '';
      anyVisible = true;
    } else if (next === tomorrow) {
      lbl.textContent = 'morgen';
      lbl.classList.add('bin-tomorrow');
      binEl.style.display = '';
      anyVisible = true;
    } else {
...
function renderLights(data) {
  ...
  if (_activeRoomId && data?.rooms?.[_activeRoomId]) {
    ...
    if (slider && document.activeElement !== slider) {
      ...
      slider.style.setProperty('--bri-pct', ((+slider.value - +slider.min) / (+slider.max - +slider.min) * 100).toFixed(2) + '%');
...
function openRoomDetail(roomId) {
  ...
  const slider = document.getElementById('lights-bri-slider');
  const briVal = document.getElementById('lights-bri-val');
  const briPct = s => ((+s.value - +s.min) / (+s.max - +s.min) * 100).toFixed(2) + '%';
  slider.style.setProperty('--bri-pct', briPct(slider));
